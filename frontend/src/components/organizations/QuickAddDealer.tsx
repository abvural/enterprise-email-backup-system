...
import { MdStorefront, MdArrowBack } from 'react-icons/md'
...
export const QuickAddDealer: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Navigate back to network view
      navigate('/distributor/network')
    } catch (error: any) {
      const errorMessage = error.response?.data?.error || 'Failed to create dealer'
      toast({
        title: 'Error',
      ...
      <Box p={8} bg="white" minH="100vh">
        <VStack align="stretch" spacing={6} maxW="600px" mx="auto">
          ...
          <VStack align="start" spacing={1}>
            ...
            <HStack spacing={3}>
              <Icon as={MdStorefront} boxSize={6} color="blue.500" />
              <VStack align="start" spacing={0}>
                <Text fontSize="2xl" fontWeight="bold" color="gray.900">
                  Add New Dealer
                </Text>
                <Text fontSize="md" color="gray.600">
                  Create a new dealer organization under your network
                </Text>
              </VStack>
            </HStack>
          </VStack>
          ...
          <Card>
            <CardHeader>
              <Alert status="info">
                <AlertIcon />
                <Text fontSize="sm">
                  Creating dealer under: <strong>{user?.primary_org?.name}</strong>
                </Text>
              </Alert>
            </CardHeader>
